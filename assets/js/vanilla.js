document.addEventListener('DOMContentLoaded', function () {
    ...
    // document.getElementById(window.zwssgrActiveTab).style.display = 'block'; // Show the active tab content

    // document.querySelectorAll('.tab-item').forEach(tab => tab.classList.remove('active'));
    // document.querySelector(`.tab-item[data-tab="${window.zwssgrActiveTab}"]`).classList.add('active');
    ...
    //     setTimeout(() => {
    //         reinitializeSlickSlider(selectedOptionDisplay);
    //     }, 100);
    // }
    ...
    document.body.addEventListener('click', function (e) {
        if (e.target && e.target.id === 'save-get-code-btn') {
            ...
            let selectedRating = document.querySelector('.star-filter.selected:last-of-type')?.dataset.rating || 0;
